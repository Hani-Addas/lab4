...
def main():
    ...
    records, captures = filter_log_by_regex(log_path, r'', print_summary=True, print_records=True)
...
def get_file_path_from_cmd_line(param_num=1):
    ...
    if num_params < param_num:
        ...
        sys.exit()
        #Get the file path and convert it to an absoulute path  
        file_path = os.path.abspath(sys.argv[param_num])

        #check wether the file exists
        if not os.path.isfile(file_path):
            print(f"Error: File '{file_path}' does not exit.")
            sys.exit()
            return file_path
